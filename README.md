...
| 3 | Runs D1 schema migrations |
| 4 | Builds Svelte dashboard |
| 5 | Deploys worker (API + dashboard + email handler) |
| 6 | Creates email routing rule (`dmarc@domain` → worker) |
| 7 | Creates `_report._dmarc` DNS TXT records (per-domain or wildcard) |
| 8 | Creates Cloudflare Access application + policy |
